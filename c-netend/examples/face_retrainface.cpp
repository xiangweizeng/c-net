...
#include "models/face/face-opt.h"
}
...
int main(int argc, char **argv) {
    ...
    network_setup_run_operations(&network_face_opt, &opt);

    /**
     * run network
     */
    session_t session = session_create(&network_face_opt);
    opt.light_mode = 1;
    session.option = opt;
    session_set_input(&session, face_opt_blob_input0_input0, input);
    ...
    outputs.data[0] = session_get_output(&session, face_opt_blob_output0_output0);
    outputs.data[1] = session_get_output(&session, face_opt_blob_530_530);
    outputs.data[2] = session_get_output(&session, face_opt_blob_529_529);
    session_release(&session);
    network_teardown_run_operations(&network_face_opt, &opt);
    ...
    while (iter != nullptr) {
        face_info_t &info = ((face_info_node *) iter)->face_info;
        Object object;
        ...
        object.landmarks[4].x = info.landmark[8] * img_w;
        object.landmarks[4].y = info.landmark[9] * img_h;
