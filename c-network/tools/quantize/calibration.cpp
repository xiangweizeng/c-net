...
static int post_training_quantize(const std::vector<std::string>& image_list, const std::string& param_path, const std::string& bin_path, const std::string& table_path, struct PreParam& per_param)
{
    ...
    {
        std::string layer_name = net.mutable_blobs()[i].name;
    ...
    for (size_t i = 0; i < image_list.size(); i++)
    {
        ...
        for (size_t i=0; i<net.mutable_blobs().size(); i++)
        {
            ...
            quantize_datas[i].initial_blob_max(out);
        }         
    }
    ...
    for (size_t i=0; i<net.mutable_blobs().size(); i++)
    {
        ...
        fprintf(stderr, "%-20s : max = %-15f interval = %-10f\n", quantize_datas[i].name.c_str(), quantize_datas[i].max_value, quantize_datas[i].histogram_interval);
    }    
    ...
    for (size_t i = 0; i < image_list.size(); i++)
    {
        ...
        for (size_t i=0; i<net.mutable_blobs().size(); i++)
        {
            ...
            ncnn::Mat out;
            ex.extract(blob_name.c_str(), out);
            quantize_datas[i].update_histogram(out);
        }     
    }
    ...
    for (size_t i=0; i<net.mutable_blobs().size(); i++)
    {
        ...
        quantize_datas[i].get_data_blob_scale();
        fprintf(stderr, "bin : %-8d "
                        ...
                        "scale : %-10f\n",
                         ...
                         quantize_datas[i].scale);
        break;
        fprintf(fp, "%s %f\n", blob_name.c_str(), quantize_datas[i].scale);
    }
...
int main(int argc, char** argv)
{
    ...
    if (!ret)
    {
        fprintf(stderr, "\nNCNN Int16 Calibration table create success, best wish for your INT16 inference has a low accuracy loss...\\(^0^)/...233...\n");
    }
