...
void free_vector_##type##_data_impl(vector_##type##_ptr vec){                   \
    if (NULL != vec->data){                                                     \
        ...
        vec->length = 0;                                                        \
                                                                                ...
                                                                                \
int get_vector_##type##_element_impl(                                           \
    vector_##type##_ptr vec,                                                    \
    long index,                                                                 \
    type *dest){                                                                \
                                                                                \
    ...
    *dest = vec->data[index];                                                   \
    return CNET_STATUS_SUCCESS;                                                 \
                                                                                ...
                                                                                \
int set_vector_##type##_element_impl(                                           \
    vector_##type##_ptr vec,                                                    \
    long index,                                                                 \
    type *src){                                                                 \
                                                                                \
...
int push_vector_##type##_element_impl(vector_##type##_ptr vec, type *src){      \
    ...
    return CNET_STATUS_SUCCESS;                                                 \
                                                                                ...
                                                                                \
void for_each_vector_##type##_element_impl(                                     \
    vector_##type##_ptr vec,                                                    \
    element_##type##_operation operation,                                       \
    void *args){                                                                \
    if(NULL == vec || NULL == operation){                                       \
        return;                                                                 \
    }                                                                           \
                                                                                \
    int i;                                                                      \
