...
static struct pthreadpool* pthreadpool_allocate(size_t threads_count) {
	...
	#else
		if (fast_memory_memalign((void**) &threadpool, PTHREADPOOL_CACHELINE_SIZE, threadpool_size) != 0) {
			return NULL;
		}
	#endif
...
struct pthreadpool* pthreadpool_create(size_t threads_count) {
	...
	if (threads_count == 0) {
		threads_count = 2;//(size_t) sysconf(_SC_NPROCESSORS_ONLN);
	}
	...
	if (threads_count > 1) {
		...
		for (size_t tid = 1; tid < threads_count; tid++) {
			int ret = pthread_create(&threadpool->threads[tid].thread_object, NULL, &thread_main, &threadpool->threads[tid]);
			if(ret != 0){
